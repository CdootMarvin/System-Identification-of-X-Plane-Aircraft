...
fInput = fopen('Data17260ktslon.txt', 'rt');
...
Qdoub = ttest.Qrad_s(27273:27850,:);
Pdoub = ttest.Prad_s(27273:27850,:);
Rdoub = ttest.Rrad_s(27273:27850,:);
thetadoub = ttest.pitchdeg(27273:27850,:)*pi/180;
thetadoub = thetadoub-thetadoub(1);
phidoub = ttest.rolldeg(27273:27850,:)*pi/180;
alphadoub = ttest.alphadeg(27273:27850,:)*pi/180;
alphadoub = alphadoub-alphadoub(1);
betadoub = ttest.betadeg(27273:27850,:)*pi/180;
vtruedoub = ttest.Vtruektas(27273:27850,:)*1.68781;
vtruedoub = vtruedoub-vtruedoub(1);
vdoub=betadoub.*vtruedoub;
wdoub=alphadoub.*vtruedoub;
Udoub=sqrt(vtruedoub.^2-vdoub.^2-wdoub.^2);
dedoub=ttest.elev1deg(27273:27850,:)*pi/180; % de 172 +-24°
dedoub=dedoub+.0752;
dadoub=(ttest.Railn1deg(27273:27850,:)/2+(-1*ttest.Lailn1deg(27273:27850,:)/2))*pi/180; %da 172 +-15
drdoub=ttest.rudr1deg(27273:27850,:)*pi/180; %dr 172 +-17
dtdoub=ttest.thro1part(27273:27850,:);
% h=ttest.alt1ftmsl;
timedoub=ttest.realtime(27273:27850,:);
...
Q = ttest.Qrad_s(30567:34121);
P = ttest.Prad_s(30567:34121);
R = ttest.Rrad_s(30567:34121);
theta = ttest.pitchdeg(30567:34121)*pi/180-ttest.pitchdeg(27273)*pi/180; %27273
phi = ttest.rolldeg(30567:34121)*pi/180;
alpha = ttest.alphadeg(30567:34121)*pi/180-ttest.alphadeg(27273)*pi/180;
beta = ttest.betadeg(30567:34121)*pi/180;
vtrue = ttest.Vtruektas(30567:34121)*1.68781-ttest.Vtruektas(27273)*1.68781;
v=beta.*vtrue;
w=alpha.*vtrue;
U=sqrt(vtrue.^2-v.^2-w.^2);
de=ttest.elev1deg(30567:34121)*pi/180; % de 172 +-24°
de = de-de(1);
da=(ttest.Railn1deg(30567:34121)/2+(-1*ttest.Lailn1deg(30567:34121)/2))*pi/180; %da 172 +-15
dr=ttest.rudr1deg(30567:34121)*pi/180; %dr 172 +-17
dt=ttest.thro1part(30567:34121);
% h=ttest.alt1ftmsl;
time=ttest.realtime(30567:34121);
timetot=ttest.totltime(30567:34121);
time = time - time(1);
...
%% OEM
% p0 = [-.1;4;.2;-0.0001;-2;1;0;0.001;-10;-2;0;-.4;-.2;-10;0];
...
Aoe = [-0.194621629423791,12.6104276693158,32.2732665010869,-32.1700000000000;-0.00480495252476786,-1.89938204051665,0.898577977783266,-0.0111713412551549;0.000955406440493272,-10.5800494795090,-1.76832928506147,0.121892298971402;0,0,1,0];
Boe = [53.7637631308541;-0.312162234720920;-11.2006724363318;0.0266668155987808];
...
figure, 
subplot(5,1,1)
plot(trsdoub,yrsdoub(:,1),'LineWidth',2),hold on, plot(trsdoub,lsimoutdoubOE(:,1),'LineWidth',2),plot(trsdoub,lsimoutdoubSINDY(:,1),'LineWidth',2)
xlabel('Time (s)')
ylabel('\Delta vtrue (ft/s)')
...
CoefsRoskam.Properties.VariableNames = {'NASA'}
...
writetable(LonStab,'SkyHawk60KtsLonStabDerivs.xlsx','WriteRowNames',true)
